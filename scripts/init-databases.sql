...
CREATE USER network_migrator    WITH PASSWORD 'network_migrator_pw';
CREATE USER network_app         WITH PASSWORD 'network_app_pw';
...
REVOKE CONNECT ON DATABASE network_db     FROM PUBLIC;
...
GRANT CONNECT, CREATE ON DATABASE network_db     TO network_migrator;
...
ALTER DATABASE network_db     OWNER TO network_migrator;
...
GRANT CONNECT ON DATABASE network_db     TO network_app;
...
CREATE EXTENSION IF NOT EXISTS pgcrypto;
GRANT USAGE ON SCHEMA public TO network_app;
ALTER DEFAULT PRIVILEGES FOR ROLE network_migrator IN SCHEMA public
