...
import { FiCheck, FiCpu, FiPlus, FiSend, FiTrash2, FiX } from 'react-icons/fi';
...
export function TranslationManagerModal({ onClose }: { onClose: () => void }) {
  ...
  const { data: keysData, isLoading } = useQuery({
    queryKey: ['translation-keys', namespace, page, debouncedSearch, onlyMissing],
    queryFn: () =>
      fetchTranslationKeys({ namespace: namespace || undefined, page, pageSize: 25, q: debouncedSearch, onlyMissing }),
  });
  ...
  // "Hepsini doldur" — görüntülenen namespace + tıklanan sütunun dili.
  const autoFillColumnMutation = useMutation({
    mutationFn: (targetLocale: string) => autoTranslateKeys({ targetLocale, namespace, onlyMissing: true }),
    onSuccess: (res) => {
      void queryClient.invalidateQueries({ queryKey: ['translation-keys'] });
                    ...
                    <div className={styles.localeHeader}>
                      ...
                      {canWrite && namespace && (
                        <button
                          ...
                          title={t('settings.translations.autoFill')}
                          onClick={() => autoFillColumnMutation.mutate(loc.code)}
                          disabled={autoFillColumnMutation.isPending}
                        >
                          <FiCpu />
                        </button>
                      )}
              ...
              {keysData?.items.map((row) => (
                <tr key={row.id}>
                  ...
                  {localesList.map((loc) => {
                    ...
                    const isMachine = trans?.updatedBy === MACHINE_ACTOR;
                    ...
                    return (
                      <td key={loc.code}>
                        ...
                        {isMachine && <span className="badge badge--blue">{t('settings.translations.machineBadge')}</span>}
                        {trans?.publishedValue && (
                          <span className={styles.publishedText} title={t('settings.translations.liveValue')}>
