...
const pool = new Pool({ connectionString: config.NETWORK_APP_DATABASE_URL });

// Boşta bekleyen bağlantı kesilmelerinde uygulamanın durmasını engellemek için havuz hata dinleyicisi.
pool.on('error', (err) => {
  console.error('[network-service] Postgres pool hatası (boşta bağlantı kesildi, devam ediliyor):', err.message);
});
