...
export async function getById(req: AuthedRequest, res: Response): Promise<void> {
  ...
  // Tekil okuma da kapsamdan geçer: liste süzülmüş olsa bile kimlik tahmin edilerek
  // doğrudan istenebilir.
  assertInScope(req.user, PERMISSIONS.NETWORK_READ, row.unitPath, id);
...
export async function getChildren(req: AuthedRequest, res: Response): Promise<void> {
  ...
  // Liste zaten süzülüyor ama ebeveyn de kapıdan geçer: aksi halde uç, kapsam dışı bir
  // kimliğin VAR OLDUĞUNU 404/200 farkıyla söyler.
  assertInScope(req.user, PERMISSIONS.NETWORK_READ, parent.unitPath, id);
