...
const targetUrl = process.env.OUTAGE_DATABASE_URL || process.env.OUTAGE_APP_DATABASE_URL;
const networkUrl = process.env.NETWORK_APP_DATABASE_URL;

if (!targetUrl) {
  console.error('OUTAGE_DATABASE_URL tanımlı değil — kök .env dosyanı kontrol et.');
  process.exit(1);
}
if (!networkUrl) {
